...
export default function SymptomPickerModal() {
  const { isSymptomModalOpen, setSymptomModalOpen, addSymptomLog, prefilledSymptomId } = useAppStore();
  ...
  useEffect(() => {
    if (isSymptomModalOpen && prefilledSymptomId) {
      const sym = symptomsData.find(s => s.id === prefilledSymptomId);
      if (sym) {
        setSelectedSymptom(sym);
      }
    } else if (!isSymptomModalOpen) {
      setSelectedSymptom(null);
      setSearch('');
      setSeverity(5);
    }
  }, [isSymptomModalOpen, prefilledSymptomId]);
  ...
  return (
    <div onClick={() => setSymptomModalOpen(false)} className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4 animate-in fade-in">
      <div onClick={(e) => e.stopPropagation()} className="bg-white w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-0 sm:zoom-in-95 flex flex-col h-[85vh] sm:h-[600px]">
        <div className="p-6 pb-4 flex justify-between items-center border-b border-gray-50">
          <h2 className="text-xl font-bold text-gray-800">
            {selectedSymptom ? 'Detail Gejala' : 'Pilih Gejala'}
          </h2>
        ...
        {!selectedSymptom ? (
          ...
          <div className="p-6 overflow-y-auto no-scrollbar flex-1 space-y-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Waktu Kejadian (Onset)</label>
              <input 
                type="datetime-local" 
                value={onset}
              ...
              />
              <p className="text-xs text-gray-400 mt-2">Kapan gejala ini pertama kali dirasakan?</p>
            </div>
